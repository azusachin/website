...
export const onRequestPut = async ({ request, env, params }) => {
  ...
  await env.DB.prepare(
    "UPDATE sections SET slug = ?, title = ?, content = ? WHERE id = ?"
  )
    .bind(data.slug, data.title, data.content, params.id)
    .run();
...
export const onRequestDelete = async ({ request, env, params }) => {
  ...
  await env.DB.prepare("DELETE FROM sections WHERE id = ?").bind(params.id).run();
  return new Response(JSON.stringify({ ok: true }), {
    headers: { "Content-Type": "application/json" } }
  );
};
